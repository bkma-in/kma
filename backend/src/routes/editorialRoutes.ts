...
async function ensureEditorialBoardSeeded() {
  try {
    const editorsSnapshot = await db.collection('editorial_board').limit(1).get();
    if (editorsSnapshot.empty) {
      ...
      for (const editor of DEFAULT_EDITORS) {
        const docRef = db.collection('editorial_board').doc();
        batch.set(docRef, {
          ...editor,
          createdAt: now,
          updatedAt: now,
      ...
      const policyRef = db.collection('editorial_policy').doc('current');
      batch.set(policyRef, {
        ...DEFAULT_POLICY,
        updatedAt: now,
      });
...
router.get('/board', async (_req, res: Response) => {
  try {
    await ensureEditorialBoardSeeded();

    const [editorsSnapshot, policyDoc] = await Promise.all([
      db.collection('editorial_board')
        .where('isActive', '==', true)
        .get(),
      db.collection('editorial_policy').doc('current').get()
    ]);
    ...
    const customCategories: { [key: string]: any[] } = {};
    ...
    const policy = policyDoc.exists ? policyDoc.data() : DEFAULT_POLICY;
    ...
    res.status(500).json({ 
      ...
      grouped: {
        core: DEFAULT_EDITORS.filter(e => e.category === 'core'),
        academic: DEFAULT_EDITORS.filter(e => e.category === 'academic'),
        associate: DEFAULT_EDITORS.filter(e => e.category === 'associate'),
        custom: {},
      },
...
router.get('/policy', async (_req, res: Response) => {
  try {
    const policyDoc = await db.collection('editorial_policy').doc('current').get();
    const policy = policyDoc.exists ? policyDoc.data() : DEFAULT_POLICY;
    res.json({ success: true, policy });
  } catch (error: any) {
    console.error('Error fetching editorial policy:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch policy', policy: DEFAULT_POLICY });
  }
});
...
router.get('/admin/board', requireAuth, requireRole(['admin']), async (_req: AuthRequest, res: Response) => {
  try {
    await ensureEditorialBoardSeeded();

    const [editorsSnapshot, policyDoc] = await Promise.all([
      db.collection('editorial_board').get(),
      db.collection('editorial_policy').doc('current').get()
    ]);
...
router.post('/admin/editors', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const {
      name,
      category,
    ...
    if (!formattedCategoryTitle) {
      if (category === 'core') formattedCategoryTitle = 'Core Editorial Team';
      else if (category === 'academic') formattedCategoryTitle = 'Academic Editors';
      else if (category === 'associate') formattedCategoryTitle = 'Associate Editors';
      else if (category === 'advisory') formattedCategoryTitle = 'Advisory Board';
      else formattedCategoryTitle = category.charAt(0).toUpperCase() + category.slice(1);
    }
    ...
    const docRef = await db.collection('editorial_board').add(newEditor);
...
router.put('/admin/editors/:id', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const {
      name,
    ...
    const editorRef = db.collection('editorial_board').doc(id);
    ...
    if (formattedCategoryTitle !== undefined) updateData.categoryTitle = formattedCategoryTitle;
    if (role !== undefined) updateData.role = role.trim();
    if (affiliation !== undefined) updateData.affiliation = affiliation.trim();
    if (email !== undefined) updateData.email = email.trim();
    if (phone !== undefined) updateData.phone = phone.trim();
    if (areas !== undefined) updateData.areas = areas.trim();
    if (profileImage !== undefined) updateData.profileImage = profileImage ? profileImage.trim() : null;
...
router.patch('/admin/editors/:id/status', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.delete('/admin/editors/:id', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const editorRef = db.collection('editorial_board').doc(id);
    const doc = await editorRef.get();
...
router.put('/admin/reorder', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const item of items) {
      if (item.id && typeof item.order === 'number') {
        const docRef = db.collection('editorial_board').doc(item.id);
        batch.update(docRef, {
          order: item.order,
          updatedAt: now,
        });
...
router.put('/admin/policy', requireAuth, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const {
      policyText,
      periodicityText,
    ...
    if (policyText !== undefined) updateData.policyText = policyText;
    if (periodicityText !== undefined) updateData.periodicityText = periodicityText;
    if (submissionGuidelines !== undefined) updateData.submissionGuidelines = submissionGuidelines;
    if (referencesFormat !== undefined) updateData.referencesFormat = referencesFormat;
    if (subscriptionText !== undefined) updateData.subscriptionText = subscriptionText;
    if (copyrightNotice !== undefined) updateData.copyrightNotice = copyrightNotice;
...
router.post('/admin/reset-policy', requireAuth, requireRole(['admin']), async (_req: AuthRequest, res: Response) => {
  try {
    const policyRef = db.collection('editorial_policy').doc('current');
    await policyRef.set({
      ...DEFAULT_POLICY,
...
router.post('/admin/seed-defaults', requireAuth, requireRole(['admin']), async (_req: AuthRequest, res: Response) => {
  try {
    // Delete existing editorial board
    const existing = await db.collection('editorial_board').get();
    const batch = db.batch();

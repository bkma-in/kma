 ...
 */
export const parseMetadata = (pageText: string): ParsedMetadata => {
  ...
  let title = '';
  const authors: ParsedMetadata['authors'] = [];
  ...
  const blocks = preAbstractText
    ...
    .filter(b => {
      ...
      if (lower.includes('bulletin') || lower.includes('kerala') || lower.includes('mathematical') || lower.includes('association')) return false;
      if (lower.includes('issn') || lower.includes('vol') || lower.includes('no.') || lower.includes('page') || lower.includes('pp.')) return false;
      return true;
    });

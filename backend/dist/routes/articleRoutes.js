...
router.delete('/:id', authMiddleware_1.requireAuth, async (req, res) => {
    try {
        ...
        if (article.pdfUrl) {
            try {
                await (0, storageService_1.deletePdfFromR2)(article.pdfUrl);
            }
            catch (err) {
        ...
        if (Array.isArray(article.revisionHistory)) {
            for (const rev of article.revisionHistory) {
                if (rev.pdfUrl) {
                    try {
                        await (0, storageService_1.deletePdfFromR2)(rev.pdfUrl);
                    }
                    catch (err) {
...
router.patch('/bulk-publish', authMiddleware_1.requireAuth, (0, authMiddleware_1.requireRole)(['admin']), async (req, res) => {
    try {
        ...
        await firebase_1.db.runTransaction(async (transaction) => {
            ...
            const docRefs = articleIds.map(id => firebase_1.db.collection('articles').doc(id));

...
exports.extractMetadataWithGemini = exports.extractFirstPageText = exports.extractPages = void 0;
 ...
 */
const extractPages = async (fullPdfBuffer, startPage, endPage) => {
    if (startPage <= 0 || endPage < startPage) {
        throw new Error(`Invalid page range: ${startPage} to ${endPage}`);
    }
    const srcDoc = await pdf_lib_1.PDFDocument.load(fullPdfBuffer);
 ...
 */
const extractFirstPageText = async (pdfBuffer) => {
    const options = {
...
exports.extractFirstPageText = extractFirstPageText;
/**
 ...
 */
const extractMetadataWithGemini = async (pageText) => {
    ...
    const model = genAI.getGenerativeModel({
        ...
        generationConfig: {
            ...
            responseSchema: {
                type: 'OBJECT',
                properties: {
                    title: { type: 'STRING', description: 'The title of the academic article.' },
                    abstract: { type: 'STRING', description: 'The abstract text of the article.' },
                    keywords: {
                        type: 'ARRAY',
                        items: { type: 'STRING' },
                    ...
                    authors: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                email: { type: 'STRING', description: 'The author\'s email address, if printed on page 1. Otherwise empty.' },
                                affiliation: { type: 'STRING', description: 'The department or institution of the author, if printed on page 1. Otherwise empty.' }
                            },
                            required: ['name']
                        },
...
exports.extractMetadataWithGemini = extractMetadataWithGemini;
